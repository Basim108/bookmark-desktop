# release-notice Specification

## Purpose
What the extension tells users about itself and about what changed in it: a
user-facing changelog kept in the repository, the current version's entry baked
into the shipped bundle, the detection of an update in the background context,
and the modal release-notice window that opens once after an update and is
reachable at any time from the settings window's About control.
