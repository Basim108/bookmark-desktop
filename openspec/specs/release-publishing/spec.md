...
## Purpose
How a version reaches users: publishing a GitHub release as the single trigger,
the agreement between the tag and the shipped manifest, the requirement that the
tagged commit already be proven by continuous integration, what gets packaged
and submitted to the Chrome Web Store, the contributor-facing release body that
accompanies it, the guard that stops a breaking change reaching users
unannounced, and the operational prerequisites for the publishing credentials
that no automation can enforce.
